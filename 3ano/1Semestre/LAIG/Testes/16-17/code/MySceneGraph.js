 ...
 */
 class MySceneGraph {
		...
		for (var i = 0; i < children.length; i++)
		{
			...
			else
			{
				...
				for (var j = 0; j < grandChildren.length; j++)
				{
					...
					else if (grandChildren[j].nodeName == "tree")
					{
						var th, tb, ch, cb, nt, tm;
						th = this.reader.getFloat(grandChildren[j], 'th');
						tb = this.reader.getFloat(grandChildren[j], 'tb');
						ch = this.reader.getFloat(grandChildren[j], 'ch');
						cb = this.reader.getFloat(grandChildren[j], 'cb');
						nt = this.reader.getInteger(grandChildren[j], 'nt');
						tm = this.reader.getString(grandChildren[j], 'tm');

						if (!(this.isValid(th) && this.isValid(tb) && this.isValid(ch) && this.isValid(cb) && this.isValid(nt) && this.isValid(tm)))
							return "Unable to parse " + children[i].nodeName + "id=\"" + primitiveId + "\" on the \"" + grandChildren[j].nodeName + "\" node";

						if (!(ch < th))
							return "ch must be lesser than th on the " + children[i].nodeName + "id=\"" + primitiveId + "\" ";

						this.primitives[primitiveId] = new Tree(this.scene, th, tb, ch, cb, nt, tm);
					}
			...
			if (children[i].nodeName == "component")
			{
				...
				for (var j = 0; j < grandChildren.length; j++)
				{
					...
					else if (grandChildren[j].nodeName == "materials")
					{
						...
						for (var k = 0; k < grandGrandChildren.length; k++)
						{
							componentsTemp[componentId][1][k] = [];
							componentsTemp[componentId][1][k][0] = this.reader.getString(grandGrandChildren[k], 'id');

							if (this.reader.hasAttribute(grandGrandChildren[k], 'invert'))
								componentsTemp[componentId][1][k][1] = this.reader.getBoolean(grandGrandChildren[k], 'invert');
							else
								componentsTemp[componentId][1][k][1] = false;

							if (!(this.isValid(componentsTemp[componentId][1][k][0])))
								return "Unable to parse material id=\"" + componentsTemp[componentId][1][k][0] + "\" on the \"" + grandChildren[j].nodeName + "\" node";
	...
	applyInvert(componentX, componentY)
	{
		if (!componentY.materials[componentY.idMaterial][1])
		{
			componentX.material.setEmission(componentY.materialParameters[1][0], componentY.materialParameters[1][1], componentY.materialParameters[1][2], componentY.materialParameters[1][3]);
			componentX.material.setAmbient(componentY.materialParameters[2][0], componentY.materialParameters[2][1], componentY.materialParameters[2][2], componentY.materialParameters[2][3]);
			componentX.material.setDiffuse(componentY.materialParameters[3][0], componentY.materialParameters[3][1], componentY.materialParameters[3][2], componentY.materialParameters[3][3]);
			componentX.material.setSpecular(componentY.materialParameters[4][0], componentY.materialParameters[4][1], componentY.materialParameters[4][2], componentY.materialParameters[4][3]);
		}
		else
		{
			componentX.material.setEmission(1-componentY.materialParameters[1][0], 1-componentY.materialParameters[1][1], 1-componentY.materialParameters[1][2], componentY.materialParameters[1][3]);
			componentX.material.setAmbient(1-componentY.materialParameters[2][0], 1-componentY.materialParameters[2][1], 1-componentY.materialParameters[2][2], componentY.materialParameters[2][3]);
			componentX.material.setDiffuse(1-componentY.materialParameters[3][0], 1-componentY.materialParameters[3][1], 1-componentY.materialParameters[3][2], componentY.materialParameters[3][3]);
			componentX.material.setSpecular(1-componentY.materialParameters[4][0], 1-componentY.materialParameters[4][1], 1-componentY.materialParameters[4][2], componentY.materialParameters[4][3]);
		}

		componentX.material.setShininess(componentY.materialParameters[0]);
	}

	fixInheritanceMaterials(componentX, componentLast)
	{
		if (componentX.materials[componentX.idMaterial][0] == "inherit")
		{
			...
			this.applyInvert(componentX, componentLast);
		}
		...
		else
		{
			...
			componentX.materialParameters = this.materials[componentX.materials[componentX.idMaterial][0]];

			this.fixInheritanceTextures(componentX, componentLast);

			this.applyInvert(componentX, componentX);
		}
