		...
		for (var i = 0; i < children.length; i++)
		{
			...
			else
			{
				...
				for (var j = 0; j < grandChildren.length; j++)
				{
					...
					else if (grandChildren[j].nodeName == "beach")
					{
						var idtexture, idheightmap, idMask, parts, heightscale;

						idtexture = this.reader.getString(grandChildren[j], 'idtexture');
						idheightmap = this.reader.getString(grandChildren[j], 'idheightmap');
						idMask = this.reader.getString(grandChildren[j], 'idMask');
						parts = this.reader.getInteger(grandChildren[j], 'parts');
						heightscale = this.reader.getFloat(grandChildren[j], 'heightscale');

						if (!(this.isValid(idtexture) && this.isValid(idheightmap) && this.isValid(idMask) && this.isValid(parts) && this.isValid(heightscale)))
							return "Unable to parse " + children[i].nodeName + "id=\"" + primitiveId + "\" on the \"" + grandChildren[j].nodeName + "\" node";

					   this.primitives[primitiveId] = new Beach(this.scene , idtexture, idheightmap, idMask, parts, heightscale);
					}
